 ...
 * Provides a high-level overview of shop performance, recent transactions,
          ...
          <div class="container flex h-16 items-center justify-between px-10">
            <div class="flex max-w-lg flex-1 items-center gap-4">
              <app-button 
                variant="outline" 
                size="sm" 
                className="h-10 w-10 p-0! app-surface app-border app-text-soft app-surface-hover" 
                (onClick)="toggleSidebar()"
              >
                <lucide-icon [name]="icons.Menu" class="size-4"></lucide-icon>
              </app-button>
              
              <div class="relative w-full max-w-xs">
                <lucide-icon [name]="icons.Search" class="absolute left-3 top-1/2 -translate-y-1/2 size-4 app-text-soft"></lucide-icon>
            ...
            <div class="flex items-center gap-3">
              <app-button variant="outline" size="sm" className="h-10 w-10 p-0! app-surface app-border app-text-soft app-surface-hover">
                <lucide-icon [name]="icons.Bell" class="size-4"></lucide-icon>
              </app-button>
              <app-button routerLink="/" variant="secondary" size="sm" className="h-10 gap-2 app-surface app-border app-text-muted app-surface-hover">
                <lucide-icon [name]="icons.ArrowRight" class="size-4"></lucide-icon>
                Shop View
              </app-button>
            </div>
          </div>
          ...
          <div class="admin-sections">
            ...
            <div class="admin-section-header flex items-center justify-between">
              ...
              <app-button variant="primary" className="bg-blue-600 hover:bg-blue-700 h-11 px-5 gap-2 text-sm font-medium rounded-xl transition-all shadow-sm">
                <lucide-icon [name]="icons.Plus" class="size-4"></lucide-icon>
                New Product Entry
              </app-button>
            </div>
...
export class AdminPageComponent implements OnInit {
  ...
  /** UI State: Recent transactions data */
  ...
  /** Async data: Dashboard metrics from API */
  ...
  /**
   * Explicitly closes the navigation sidebar.
   */
  closeSidebar() {
  ...
  /**
   * Returns CSS classes for transaction status badges.
   * Why: Centralizes styling logic for status badges across the admin UI.
  ...
  /**
   * Determines the appropriate icon for a stat card.
   * Why: Maps business metric labels to visual representations.
  ...
  /**
   * Returns color classes for stat cards based on the metric type.
   * @param label Stat label
   */
