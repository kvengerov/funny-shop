...
/**
 * Feature component providing the user login interface.
 * Handles form validation, credential submission, and loading/error states.
 */
@Component({
...
export class LoginFormComponent {
  ...
  /** Initialized login form with validation rules */
  loginForm = this.fb.group({
    username: ['', [Validators.required, Validators.minLength(3)]],
    password: ['', [Validators.required, Validators.minLength(6)]]
  });

  /** Reactive state tracking submission status */
  isLoading = signal(false);

  /** Reactive state tracking authentication errors */
  ...
  /**
   * Generates user-friendly error messages based on control validation errors.
   * @param controlName Name of the form control to validate
   * @returns Error message string or null if valid/untouched
   */
  ...
  /**
   * Handles form submission and initiates authentication.
   * Why: Prevents default browser reload and manages async loading/error flow.
