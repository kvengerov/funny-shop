...
/**
 * Feature component providing the user registration interface.
 * Handles form validation, account creation requests, and UI states.
 */
@Component({
...
export class RegisterFormComponent {
  /** Reactive form builder for constructing the registration form */
  private fb = inject(FormBuilder);

  /** Authentication API service for interacting with the backend */
  private authApi = inject(AuthApiService);

  /** Registration form configuration with validation schemas */
  ...
  /** Reactive state tracking registration submission status */
  isLoading = signal(false);

  /** Reactive state tracking registration errors */
  error = signal<string | null>(null);

  /** Emitted upon successful account creation */
  onSuccess = output<any>();

  // Lucide icons for template usage
  protected User = User;
  protected Mail = Mail;
  ...
  /**
   * Generates localized error messages based on validation states.
   * @param controlName Name of the form field to validate
   * @returns Formatted error string or null if the field status is OK
   */
  ...
  /**
   * Initiates the registration process with validated form data.
   * Why: Centralizes validation checks and error handling for the registration flow.
