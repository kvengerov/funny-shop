 ...
 * Provides links to main dashboard sections and user session management.
 */
@Component({
      ...
      <div class="admin-sidebar-inner flex h-full flex-col">
        ...
        <div class="admin-sidebar-header flex h-16 items-center px-3">
          <div class="flex items-center gap-3">
            <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-600 text-white shadow-lg shadow-blue-600/30">
              <lucide-icon [name]="icons.LayoutDashboard" class="size-5"></lucide-icon>
            </div>
            <span class="text-lg font-bold tracking-tight app-text-main">Funny Admin</span>
          </div>
          <button 
            class="admin-close flex h-8 w-8 items-center justify-center rounded-full border border-transparent app-text-soft hover:bg-white/5 hover:text-white transition-colors lg:hidden"
            (click)="onClose.emit()"
            aria-label="Close sidebar"
          >
            <span class="sr-only">Close sidebar</span>
            <lucide-icon [name]="icons.X" class="size-4"></lucide-icon>
          </button>
        </div>
        ...
        <nav class="admin-sidebar-nav flex-1 px-3 py-6">
          ...
          <a routerLink="/admin" routerLinkActive="app-active" [routerLinkActiveOptions]="{exact: true}"
            ...
            <span class="nav-icon">
              <lucide-icon [name]="icons.LayoutDashboard" class="size-4"></lucide-icon>
            </span>
          ...
          <a routerLink="/admin/products" routerLinkActive="app-active"
             class="admin-nav-link flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold app-text-muted transition-colors">
            <span class="nav-icon">
              <lucide-icon [name]="icons.ShoppingBag" class="size-4"></lucide-icon>
            </span>
          ...
          <a routerLink="/admin/customers" routerLinkActive="app-active"
             class="admin-nav-link flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold app-text-muted transition-colors">
            <span class="nav-icon">
              <lucide-icon [name]="icons.Users" class="size-4"></lucide-icon>
            </span>
          ...
          <a routerLink="/admin/stats" routerLinkActive="app-active"
             class="admin-nav-link flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold app-text-muted transition-colors">
            <span class="nav-icon">
              <lucide-icon [name]="icons.BarChart3" class="size-4"></lucide-icon>
            </span>
          ...
          <a routerLink="/admin/settings" routerLinkActive="app-active"
            ...
            <span class="nav-icon">
              <lucide-icon [name]="icons.Settings" class="size-4"></lucide-icon>
            </span>
        ...
        <div class="admin-sidebar-footer p-3">
          <div class="admin-profile flex items-center gap-3 rounded-2xl px-3 py-2.5">
            ...
            <button 
              class="admin-logout ml-auto h-8 w-8 rounded-full flex items-center justify-center app-text-soft hover:text-white transition-colors hover:bg-white/5"
              aria-label="Logout"
            >
              <lucide-icon [name]="icons.LogOut" class="size-4"></lucide-icon>
            </button>
          </div>
        </div>
      </div>
...
export class AdminSidebar {
  /** Service providing current user session details */
  protected readonly sessionService = inject(SessionService);

  /** Whether the sidebar is currently visible (primarily for mobile) */
  isOpen = input(true);
